...
import CIUSuite3.CIU_raw as CIU_raw
import CIUSuite3.Raw_Processing as Raw_Processing
import CIUSuite3.CIU_analysis_obj as CIU_analysis_obj
...
def main_gaussian_lmfit_wrapper(analysis_obj_list, params_obj, outputpath):
    ...
    num_cores = 1
    params_obj.gaussian_71_max_prot_components = 1
    if num_cores > 1:
...
def compute_area_penalty(gaussian, list_of_gaussians, dt_axis, penalty_mode):
    ...
    my_area = scipy.integrate.trapezoid(gaussfunc(dt_axis, *gaussian.return_popt()), dt_axis)
...
def shared_area_gauss(x_axis, gauss1_params, gauss2_params):
    ...
    return scipy.integrate.trapezoid(shared_area_arr, x_axis)
