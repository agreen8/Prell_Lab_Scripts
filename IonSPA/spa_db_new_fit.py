...
def find_all_rids(dbname, cid, iid):
    tnames = 'rid,cid,iid,Vin,tmax'
...
def main(celld, iond, dH, dS, Vins, fracs, title='', outfile=None, plotfile=None, weighting='combined', tpoints=0, updatedb=False, Tfactor=1.0):
    ...
    start_get_data_time = time()
    all_rids, grabbed_Vins = find_all_rids(gdbname, cid, iid)
    # print(f'All rids: {all_rids}', flush=True)
    # print(Vins)
    # print(grabbed_Vins)
    if grabbed_Vins.sort() == Vins.sort() and len(all_rids) == len(Vins):
        print('Got all Vins and rids!', flush=True)
    else:
        temp_rids = []
        temp_Vins = []
        for exp_Vin in Vins:
            if exp_Vin not in grabbed_Vins:
                print('THERE IS MISSING VINS! CANNOT PERFORM FIT', flush=True)
                return 0,0,0
        for rid, gVin in zip(all_rids, grabbed_Vins):
            if gVin in Vins:
                temp_rids.append(rid)
                temp_Vins.append(gVin)
        all_rids = temp_rids
        grabbed_Vins = temp_Vins
    
    trajs_df = get_all_trajs(gdbname)
    tTd = {}
    tTd95 = {}
    
    for rid, Vin in zip(all_rids, grabbed_Vins):
        sub_df = trajs_df[trajs_df['rid'] == rid]
        dfv, dfv95 = new_get_tzT(sub_df, tpoints)
        tTd[Vin] = dfv
        tTd95[Vin] = dfv95
    end_get_data_time = time()
    get_data_time = 1000*(end_get_data_time - start_get_data_time)
    print(f'Got the trajectory data in {get_data_time:0.2f} ms')
...
#    print(jfile, dH, dS)
    ...
    print(fracs)
    title = f'{jfile} '
    main(celld, iond, dH, dS, Vins, fracs, title, outfile, plotfile, weighting, tpoints, updatedb)
