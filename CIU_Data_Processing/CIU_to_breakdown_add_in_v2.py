# -*- coding: utf-8 -*-
"""
Created on Thu Apr  2 09:18:36 2026
...
date_name = date.today().strftime("%Y%m%d")
main_out_path = dir_path + r'\CIU_to_Breakdown_OUTPUTS\\'

if not os.path.exists(main_out_path):        #Creates Folder for Breakdown curve output if one isn't already created
...
protein_code = Breakdown_Curve_Settings['protein_code'] #Eventually, we can have it manually get the code from the filename
...
low_dt_percent = protein_dict["low_dt_bound"]
high_dt_percent = protein_dict["high_dt_bound"]
other_dt_percent = protein_dict["other_dt_bound"]
...
add_in = protein_dict['add_in']
add_std = protein_dict['add_std']
subtract = True #always true
ccs = True
extra_name = protein_dict['extra_name']

date_folder_name = main_out_path + date_name + r'_' + protein_code + r'_' + extra_name
out_path = date_folder_name + r'\Breakdown_curves'
...
output_yaml_input_file = date_folder_name + r'\input_yaml_file.json'
with open(output_yaml_input_file, 'w') as f:
...
try:
    ...
    for key1 in gaussian_data.keys():
        ...
        for i in range(num_feats):
            ...
            gaussian_data[key1][f'Feature {i+1}']['GAUSSIAN_AMP'] = []
            gaussian_data[key1][f'Feature {i+1}']['GAUSSIAN_CENTROID'] = []
            gaussian_data[key1][f'Feature {i+1}']['GAUSSIAN_STD'] = []
            gaussian_data[key1][f'Feature {i+1}']['LEFTOVER_AUC'] = []
            gaussian_data[key1][f'Feature {i+1}']['RIGHT_AUC'] = []
            gaussian_data[key1][f'Feature {i+1}']['ADD_IN'] = []
        ...
        # break
                ...
                if 'SF6' in key1 or '_N2_ ' in key1 or ccs:         #just catches a couple of edgecases for the 6560c BSA data
                    if exp_centroid > 12000 or exp_centroid < 15:
                        continue
                    if exp_fwhm < 3:
                        exp_fwhm = 1.5
                        # continue
        ...
        for i in range(num_feats):  #ok this sets all those zeros to the median FWHM
            ...
            gaussian_data[key1][f'Feature {i+1}']['FWHMs'] = list(fwhms)
        ...
        plot_file_name = date_folder_name + '\\' + key1 + r'_plots_addinv2_UE5.pdf'
        
        pp = PdfPages(plot_file_name)  #creates the PDF that will store all the gaussian fits and the BDCS
        ...
        for v in range(len(new_Vins)):
            ...
            # if round(interp_vin) != 41:
            #     continue
            print(f'Vin: {interp_vin}')
            
            smoothed_abund = smoothed_new_abundances[v]     #this is the total abundance that everything grabs from 
            ...
            for i in range(num_feats-1, -1, -1):    #this fits the second to last feature first, and then descends to the first feature (in general I've found this reverse order to be better)
                ...
                if i == 0: # if it is the first feature use the lower bound as a percentage, the upper is a percentage of the drift time distance to the centroid of the next feature
                    lower_dt = centroid*0.1
                    higher_dt = abs(centroid - gaussian_data[key1][f'Feature {i+2}']['Median Centroid'])*high_dt_percent
                    
                elif i == num_feats-1: # otherwise if it's the last feature then do the reverse 
                    higher_dt = centroid*0.1
                    lower_dt = abs(centroid - gaussian_data[key1][f'Feature {i}']['Median Centroid'])*low_dt_percent
                else:
                    lower_dt = abs(centroid - gaussian_data[key1][f'Feature {i}']['Median Centroid'])*low_dt_percent
                    higher_dt = abs(centroid - gaussian_data[key1][f'Feature {i+2}']['Median Centroid'])*high_dt_percent
                ...
                # This bit of code is for finding the 'fitting' abundance, I restrict the abundance that is being fit to help the gaussians not spread out as much
                ...
                if i == 0:
                    area_l_dt = centroid - 3*std
                else:
                    area_l_dt = (centroid + gaussian_data[key1][f'Feature {i}']['Median Centroid'])/2
                    # area_l_dt = centroid - (centroid - gaussian_data[key1][f'Feature {i}']['Median Centroid'])/4
    
                lower_append_dt_index = np.argmin(np.abs((new_drifttimes - area_l_dt)))
                higher_append_dt_index = np.argmin(np.abs((new_drifttimes - area_h_dt))) 
                
                append_abund = smoothed_abund[lower_append_dt_index:higher_append_dt_index]
                append_drift = new_drifttimes[lower_append_dt_index:higher_append_dt_index]
                ...
                gaussian_data[key1][f'Feature {i+1}']['GAUSSIAN_AMP'].append(fit_amp)
                gaussian_data[key1][f'Feature {i+1}']['GAUSSIAN_CENTROID'].append(fit_centroid)
                gaussian_data[key1][f'Feature {i+1}']['GAUSSIAN_STD'].append(fit_sigma)
                
                # leftover_abund = np.array(fit_abund)
                leftover_abund = smoothed_abund - gaussian      #subracts the fitted gaussian from the whole abundance curve
                ...
                left_leftover_index = np.argmin(np.abs((fit_drift - (fit_centroid - 0*fit_sigma))))
                right_leftover_index = np.argmin(np.abs((fit_drift - (fit_centroid + 2*fit_sigma))))
                
                population_a = fit_abund[left_leftover_index:right_leftover_index]

                gauss_auc = integrate.trapezoid(gaussian, new_drifttimes)      #integrating under the curve for the abundance data
                popa_auc = integrate.trapezoid(population_a, fit_drift[left_leftover_index:right_leftover_index])
                add_auc = 0

                for z in add_in:
                    if i+1 == z:
                        print('hi')
                        old_centroid = gaussian_data[key1][f'Feature {i+2}']['GAUSSIAN_CENTROID'][-1]
                        old_sigma = gaussian_data[key1][f'Feature {i+1}']['GAUSSIAN_STD'][-1]
                        old_amp = gaussian_data[key1][f'Feature {i+1}']['GAUSSIAN_AMP'][-1]
                        old_gaussian = plot_gauss_func(new_drifttimes, old_amp, old_centroid, old_sigma)
                        
                        add_in_abund = leftover_abund - old_gaussian
                        add_in_abund = np.where(add_in_abund <= 0, 1, add_in_abund)   # setting all abundances to be positive
                        
                        l_addin_bound = np.argmin(np.abs((new_drifttimes - (fit_centroid + 2*fit_sigma)))) #this makes it start at the center of the new fitted gaussian
                        r_addin_bound = np.argmin(np.abs((new_drifttimes - (old_centroid - 0*old_sigma)))) 

                        add_auc = integrate.trapezoid(add_in_abund[l_addin_bound:r_addin_bound], new_drifttimes[l_addin_bound:r_addin_bound])

                        
                        gaussian_data[key1][f'Feature {i+1}']['ADD_IN'].append(add_auc)

                        fit_centroid = new_drifttimes[r_addin_bound] #- 1*fit_sigma 
                        fit_sigma = 0
                        ax.fill_between(new_drifttimes[l_addin_bound:r_addin_bound], add_in_abund[l_addin_bound:r_addin_bound], 0, color=colors[i])


                left_everythingelse_index = np.argmin(np.abs((new_drifttimes - (fit_centroid + 3*fit_sigma))))

                population_b = leftover_abund[left_everythingelse_index:]
                popb_auc = integrate.trapezoid(population_b, new_drifttimes[left_everythingelse_index:])
                prec_auc = gauss_auc + popa_auc + add_auc
                ...
                ax.fill_between(fit_drift[left_leftover_index:right_leftover_index], population_a, 0, color='r')
                ax.fill_between(new_drifttimes[left_everythingelse_index:], population_b, 0, color='k')
        ...
        for i in range(num_feats):
            ...
            gaussian_data[key1][f'Feature {i+1}']['BDC'] = list(bdc)
            ...
            if 'BSA' in key1:
                protein = 'BSA'
                buffer = ''
                if 'z17' in key1 or '_17_' in key1 or '3900' in key1 or 'BSA17' in key1:
                    charge = 17
                if 'z16' in key1 or '_16_' in key1 or 'BSA16' in key1:
                    charge = 16
                if 'z15' in key1 or '_15_' in key1 or 'bsa15' in key1 or 'BSA15' in key1:
                    charge = 15
            ...
            if 'NISTmAb' in key1 or 'NIST_mAb' in key1 or 'NistmAb' in key1:
                protein = 'NistmAb'
                feature = i + 1
                buffer = ''
                if r'#29' in key1 or 'mAb13' in key1:
                    charge = 13 
                if r'#29' in key1 or 'mAb14' in key1:
                    charge = 14 
                if r'#29' in key1 or 'mAb19' in key1:
                    charge = 19 
                if r'#29' in key1 or 'mAb20' in key1:
                    charge = 20 
                if r'#24' in key1 or 'mAb24' in key1:
                    charge = 24
                if r'_25_' in key1 or 'mAb25' in key1:
                    charge = 25
                if r'#26' in key1 or 'mAb26' in key1:
                    charge = 26
                if r'#27' in key1 or 'mAb27' in key1:
                    charge = 27
                if r'#28' in key1 or 'mAb28' in key1:
                    charge = 28 
                if r'#29' in key1 or 'mAb29' in key1:
                    charge = 29 
                
                if r'a.txt' in key1 or r'A.txt' in key1 or r'DA.raw' in key1:
                    trip = 'A'
                if r'b.txt' in key1 or r'B.txt' in key1 or r'GA.raw' in key1:
                    trip = 'B'
                if r'c.txt' in key1 or r'C.txt' in key1 or r'JA.raw' in key1:
                    trip = 'C'
    ...
    output_gaussian_dic_file = date_folder_name + r'\gaussian_dictionary.json'
    with open(output_gaussian_dic_file, 'w') as f:
        json.dump(gaussian_data, f, indent=4)
