# -*- coding: utf-8 -*-
"""
Created on Fri Feb 27 12:15:15 2026
...
out_path = dir_path + r'\Rohan_ConA_Breakdown_curves'
...
for key1 in gaussian_data.keys():
    ...
    pp = PdfPages(f"{key1}_plots_Rohan_ConA_UE5.pdf")  #creates the PDF that will store all the gaussian fits and the BDCS
        ...
        for i in range(num_feats-1, -1, -1):    #this fits the second to last feature first, and then descends to the first feature (in general I've found this reverse order to be better)
            ...
            for z in add_in:
                if i+1 == z:
                    ...
                    l_bound = np.argmin(np.abs((new_drifttimes - (fit_centroid + 1*fit_sigma))))
                    r_bound = np.argmin(np.abs((new_drifttimes - (old_centroid - 3*old_sigma))))
