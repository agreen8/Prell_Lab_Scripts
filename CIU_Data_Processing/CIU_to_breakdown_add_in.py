...
try:
    ...
    for key1 in gaussian_data.keys():
        ...
        plot_file_name = date_folder_name + '\\' + key1 + r'_plots_Rohan_ConA_UE5.pdf'
        ...
        for v in range(len(new_Vins)):
            ...
            for i in range(num_feats-1, -1, -1):    #this fits the second to last feature first, and then descends to the first feature (in general I've found this reverse order to be better)
                ...
                if i == 0: # if it is the first feature use the lower bound as a percentage, the upper is a percentage of the drift time distance to the centroid of the next feature
                    ...
                    higher_dt = abs(centroid - gaussian_data[key1][f'Feature {i+2}']['Median Centroid'])*high_dt_bound
                    
                elif i == num_feats-1: # otherwise if it's the last feature then do the reverse 
                    higher_dt = centroid*high_dt_bound
                    lower_dt = abs(centroid - gaussian_data[key1][f'Feature {i}']['Median Centroid'])*high_dt_bound
                else:
                    lower_dt = abs(centroid - gaussian_data[key1][f'Feature {i}']['Median Centroid'])*high_dt_bound
                    higher_dt = abs(centroid - gaussian_data[key1][f'Feature {i+2}']['Median Centroid'])*high_dt_bound
                ...
                for z in add_in:
                    if i+1 == z:
                        # print('hi')
                        if i+1 == num_feats:
                            old_centroid = gaussian_data[key1][f'Feature {i+2}']['Median Centroid']
                            old_fwhm = gaussian_data[key1][f'Feature {i+1}']['Median FWHM']
                            old_sigma = old_fwhm / (2 * np.sqrt(2*np.log(2))) 
                        else:
                        ...
                        l_bound = np.argmin(np.abs((new_drifttimes - (fit_centroid + 0*fit_sigma))))
                        r_bound = np.argmin(np.abs((new_drifttimes - (old_centroid - 1*old_sigma))))
                        c_bound = np.argmin(np.abs((new_drifttimes - (old_centroid - 0*old_sigma))))
                        old_gauss = plot_gauss_func(new_drifttimes, leftover_abund[c_bound], old_centroid, old_sigma)
                        # leftover_abund = leftover_abund - old_gauss
                        # leftover_abund = np.where(leftover_abund <= 0, 1, leftover_abund)   # setting all abundances to be positive
                        add_auc = integrate.trapezoid(leftover_abund[l_bound:r_bound], new_drifttimes[l_bound:r_bound])
                        
                        # break
                        
                        # if i == 0: # if it is the first feature use the lower bound as a percentage, the upper is a percentage of the drift time distance to the centroid of the next feature
                            # lower_dt = centroid #+ abs(centroid - gaussian_data[key1][f'Feature {i+2}']['Median Centroid'])#*amp_dt_bound
                        #     higher_dt = gaussian_data[key1][f'Feature {i+2}']['Median Centroid']
                        # elif i == num_feats-1: # otherwise if it's the last feature then do the reverse 
                        #     higher_dt = centroid*1.5
                        #     lower_dt = centroid#gaussian_data[key1][f'Feature {i}']['Median Centroid']
                        # else:
                        #     lower_dt = centroid
                        #     higher_dt = gaussian_data[key1][f'Feature {i+2}']['Median Centroid']
                        # add_popt, add_pcov = curve_fit(plot_gauss_func, fit_drift, fit_abund, bounds=([amplitude*amp_low,lower_dt,std*std_low], [amplitude*1.25,higher_dt,std*add_std]), maxfev=25000)
                        # add_amp, add_centroid, add_sigma = add_popt
                        # add_gaussian = plot_gauss_func(fit_drift, add_amp, add_centroid, add_sigma)
                        # add_sub_gaussian = plot_gauss_func(new_drifttimes, add_amp, add_centroid, add_sigma)
                        
                        # ax.plot(fit_drift, fit_abund, color='y', ls=':', )
                        # fit_abund = np.array(fit_abund) - add_gaussian
                        # fit_abund = np.where(fit_abund <= 0, 1, fit_abund)   # setting all abundances to be positive
                        # fit_abund = list(fit_abund)
                        
                        # leftover_abund = leftover_abund - add_sub_gaussian
                        # leftover_abund = np.where(leftover_abund <= 0, 1, leftover_abund)   # setting all abundances to be positive
                        
                        # add_auc = integrate.trapezoid(add_gaussian, fit_drift)
                        
                        # # print(fit_centroid, add_centroid)
                        
                        gaussian_data[key1][f'Feature {i+1}']['ADD_IN'].append(add_auc)
                        
                        fit_centroid = new_drifttimes[r_bound] - 1*fit_sigma 
                        # fit_sigma = add_sigma
                        ...
                        ax.fill_between(new_drifttimes[l_bound:r_bound], leftover_abund[l_bound:r_bound], 0, color=colors[i])
                # if i ==0:
                #     break
                right_leftover_index = np.argmin(np.abs((new_drifttimes - (fit_centroid + 1*fit_sigma))))
